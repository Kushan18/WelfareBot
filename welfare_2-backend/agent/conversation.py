...
def _ask_field(field: str) -> Dict[str, Any]:
    return {
        ...
        "chips": _format_chips(PROFILE_CHIPS.get(field, [START_OVER])),
...
def _confirm_response(user_doc: Dict[str, Any]) -> Dict[str, Any]:
    return {
        ...
        "chips": _format_chips(["Yes", "Edit", START_OVER]),
...
def handle_turn(session_id, message, users_collection, schemes_collection, welfare_graph) -> Dict[str, Any]:
    ...
    def _format_chips(chips: List[str]) -> List[Dict[str, str]]:
        """Convert a list of chip labels into ChatResponse Chip objects.

        Each chip will have the same string for both label and value.
        """
        return [{"label": c, "value": c} for c in chips]
    ...
    if step == "name" or not user_doc.get("name"):
        ...
        return {
            ...
            "chips": _format_chips(LANGUAGE_CHIPS + [START_OVER]),
            "intent": "provide_name",
        }
    ...
    if step == "language":
        ...
        # Generate language chips with ISO codes + start over
        language_chips = [{"label": lab, "value": LANGUAGE_CODE_MAP.get(lab, lab[:2].lower())} for lab in LANGUAGE_CHIPS]
        language_chips.append({"label": START_OVER, "value": START_OVER})
        return {
    ...
    if step == "mode":
        if "form" in lower:
            ...
            return {
                ...
                "chips": _format_chips([START_OVER]),
                "open_form": True,
                "intent": "onboarding",
            }
        if "chat" in lower:
            ...
            return {
                ...
                "chips": _format_chips(chips_general_chat()),
                "intent": "onboarding",
            }
        return {
            "reply": "Would you like to fill a form or just chat?",
            "chips": _format_chips(["Fill Form", "Chat Instead", START_OVER]),
            "show_form_choice": True,
            "intent": "onboarding",
        }
    ...
    if step.startswith("collect_"):
        ...
        if nxt:
            ...
            return {"reply": r["reply"], "chips": _format_chips(r["chips"]), "intent": "onboarding"}
    ...
    if step == "confirm":
        if "edit" in lower:
            ...
            return {"reply": "Which detail would you like to edit?", "chips": _format_chips(EDIT_FIELD_CHIPS), "intent": "confirm_details"}
        if any(w in lower for w in YES_WORDS):
            save({"onboarding_step": "ready"})
            return {"reply": "All set! I will now look for schemes that match you.", "intent": "completed"}
        return {**_confirm_response(user_doc), "intent": "confirm_details"}
    ...
    if step == "edit_select":
        ...
        if not field:
            return {
                ...
                "chips": _format_chips(EDIT_FIELD_CHIPS),
                "intent": "onboarding",
            }
